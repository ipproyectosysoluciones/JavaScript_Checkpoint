...
const calcularImpuestos = (factura, i = 0, acc = 0) => {
  ...
  if ( !Array.isArray( factura ) ) return 'Error';  // Si la factura no es un array, debes retornar "Error".

  if ( factura.length === 0 ) return false;   // Si no tiene impuestos, debes retornar false.

  if ( i === factura.length ) return acc;

  if ( typeof factura[ i ] !== 'number' ) return 'Error';

...
BinarySearchTree.prototype.getHouseValues = function (side, total = 0) {
  ...
  if (this.left && side === "left") {
    total += this.left.getHouseValues(side, total);
  }
  if (this.right && side === "right") {
    total += this.right.getHouseValues(side, total);
  }
  if ( !this.left && !this.right ) return this.value;
  
  if ( this.left && this.right ) return this.left.getHouseValues( side, total ) + this.right.getHouseValues( side, total );
  
  if ( this.left && side === "right" ) return this.left.getHouseValues( side, total );
  
  if (this.right && side === "left") return this.right.getHouseValues(side, total);
